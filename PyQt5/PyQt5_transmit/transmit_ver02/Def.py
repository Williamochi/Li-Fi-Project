...
def YCbCr_to_RGB(Y, Cb, Cr, h, w):
    temp_combin = [[[0 for k1 in range(3)] for k2 in range(h*8)] for k3 in range(w*8)]
    for i in range(h*8):
        for j in range(w*8):
            temp_combin[i][j][0] = Y[i][j] + 1.772*(Cb[i][j] - 128)                           #B
            temp_combin[i][j][1] = Y[i][j] - 0.34414*(Cb[i][j] - 128) - 0.71414*(Cr[i][j] - 128)   #G
            temp_combin[i][j][2] = Y[i][j] + 1.402*(Cr[i][j] - 128)                           #R
    temp_combin = np.round_(temp_combin,0)
    return temp_combin

def divide_string(s):
    temp = '1'+'0'*20 + '1' + '2'
    n = len(s)
    chunk_size = (n + 3) // 4  # Round up to the nearest multiple of 4
    chunks = [s[i:i+chunk_size] for i in range(0, n, chunk_size)]
    if len(chunks) == 3:
        groups = [f'{chunk}{temp}' for i, chunk in enumerate(chunks)]
        return ''.join(groups)
    else:
        # Put any leftover characters into the last group
        last_chunk = chunks[-1] + ''.join(chunks[4:])
        groups = [f'{chunk}{temp}' for i, chunk in enumerate(chunks[:3])] + [f'{last_chunk}']
        return ''.join(groups)
